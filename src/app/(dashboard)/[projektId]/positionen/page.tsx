...
export default function PositionenListPage() {
    ...
    useEffect(() => {
        const timer = setTimeout(() => {
            // For the prototype, we just get all positionen and let the store filter if it could
            // Here we just get all and filter manually if needed, but mockStore.getPositionen() works
            setItems(mockStore.getPositionen());
            setLoading(false);
        }, 500);
                                                ...
                                                <Button
                                                    ...
                                                    onClick={() => {
                                                        if (confirm(`Sind Sie sicher, dass Sie "${item.name}" löschen möchten?`)) {
                                                            const currentItems = mockStore.getPositionen(); // Assuming this returns all
                                                            const newItems = currentItems.filter((i: Position) => i.id !== item.id);
                                                            // Note: mockStore.savePositionen isn't explicitly exposed in snippet but might exist or be needed
                                                            // mockStore.savePositionen(newItems); 
                                                            setItems(newItems);
                                                        }
                                                    }}
                                                >
