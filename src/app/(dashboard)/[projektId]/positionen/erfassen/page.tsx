...
export default function PositionErfassenPage() {
    ...
    const onSubmit = async (data: PositionValues) => {
        ...
        const newItem = {
            id: `pos-${Date.now()}`,
            ...data,
            teilsystemId: teilsystemId || data.teilsystemId, // ensure teilsystemId is set
            status: data.status as any
        };
        // Mock saving
        // mockStore.savePositionen([...all, newItem]); 
        console.log("Saving new position:", newItem);
        alert("Position gespeichert (Mock)"); // Temporary feedback as mockStore.savePositionen might not exist on all clients yet
...
// Add savePositionen to store if not there
// We should update store.ts as well.
