...
export default function MitarbeiterDetailPage() {
    ...
    React.useEffect(() => {
        const allMitarbeiter = mockStore.getMitarbeiter();
        const found = allMitarbeiter.find((m: any) => m.id === id);
                        ...
                        <Button
                            ...
                            onClick={() => {
                                if (confirm(`Sind Sie sicher, dass Sie "${mitarbeiter.vorname} ${mitarbeiter.nachname}" löschen möchten?`)) {
                                    const allMitarbeiter = mockStore.getMitarbeiter();
                                    const updated = allMitarbeiter.filter((m: any) => m.id !== id);
                                    mockStore.saveMitarbeiter(updated);
