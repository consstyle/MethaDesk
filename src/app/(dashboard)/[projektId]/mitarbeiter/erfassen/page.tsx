...
export default function MitarbeiterErfassenPage() {
    ...
    const onSubmit = async (data: MitarbeiterValues) => {
        ...
        const all = mockStore.getMitarbeiter();
        const newItem = {
            id: `mit-${Date.now()}`,
            ...data,
        ...
        // Note: mockStore might need a saveMitarbeiter function
        if (mockStore.saveMitarbeiter) {
            mockStore.saveMitarbeiter([...all, newItem]);
        } else {
            // Fallback or add it to store
            localStorage.setItem('methabau_mitarbeiter', JSON.stringify([...all, newItem]));
        }
